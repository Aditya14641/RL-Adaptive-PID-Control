...
min_gains = [-5.0, -10.0, 0.0]
max_gains = [20.0, 20.0, 2.0]
...
parser.add_argument("--early_stopping_limits", default=5000)
...
limits = int(args.early_stopping_limits)
extra = f"BetterES_SystemFix"
tag_name = f"CS3_{env_model}_{algo}_AR_{action_repeat}_{action_repeat_value}_use_sde_{use_sde}_ES_{early_stopping}_extra_{extra}"
print("Run Name: ", tag_name)

base_log = args.logdir
log_dir = os.path.join(base_log, "CS3", tag_name)
...
if early_stopping:
    if algo in ["DDPG", "A2C"]:
        env = EarlyStopping(env, [-limits, limits], 1)
        print("Using modified Early Stopping Criteria")
    else:
